        ...
        internal static void CheckConnectionString(string fileName)
        {
            ...
            { // Full path
                if (!File.Exists(fileName))
                    throw new Exception("ASP.NET_Access_Providers2 File not found: " + fileName);
                return;
            }
            char c = fileName[0];
            if (c == '/' || c == '\\')
            {
                throw new Exception("ASP.NET_Access_Providers2 File can not start with this char: " + c);
            }
